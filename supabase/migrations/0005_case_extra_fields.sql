...
--   施主名       -> owner_name            (② column M)
-- 製造完了 (② column J, manually-entered "完") also confirmed missing.
-- 決定金額 (② column L) and column N were considered but confirmed NOT
-- needed (user: no pricing data in this app; column N doesn't exist for
-- real) — intentionally not added here.
...
alter table design_cases add column if not exists owner_name text not null default '';
