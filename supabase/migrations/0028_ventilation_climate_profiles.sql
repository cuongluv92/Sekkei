...
-- 2023年版) に含まれる東京の値のみ (ファイルのシート構成は東京の屋外
-- フィルタ有無2種と、地域共通の屋内フィルタ有無2種のみで、他地域のシートは
-- 存在しない)。他の地域を計算する場合は JSIA-T1016 原本または自社の設計
-- 基準にある値を確認のうえ、設定画面から追加登録すること (seismic_anchor_
-- bolt_allowables 等と同じ社内選定マスタの方針)。
