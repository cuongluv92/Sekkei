 ...
 * 母線銅帯 and 接地線 have both graduated out of this registry into their own
 * real modules (see `/calculations/busbar`, `/calculations/earth-wire`) —
 * this now only holds modules without a real, standard-backed formula yet,
 * and is currently empty. Adding another placeholder module means appending
 * one entry here plus a matching `CalculationDefinition` in
 * `lib/mock/calculationDefinitions.ts`; a module with a real formula should
 * follow 母線銅帯/接地線/重量計算's bespoke-component pattern instead.
 */
const OTHER_CALC_MODULES: { key: string; labelKey: string }[] = [];

export default function OtherCalculationPage() {
  const { t } = useTranslation();
  const [active, setActive] = useState<string | null>(
    OTHER_CALC_MODULES[0]?.key ?? null,
  );
  const activeModule = OTHER_CALC_MODULES.find((m) => m.key === active);
