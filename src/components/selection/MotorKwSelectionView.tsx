            ...
            <table className="data-table table-fixed" style={{ minWidth: 900 }}>
              ...
              <tbody>
                {[
                  ...
                  ["定格電流", naisenBreakerA != null ? `${naisenBreakerA} A` : "—", rowByBasis.company?.breakerRatedA != null ? `${rowByBasis.company.breakerRatedA} A` : "—", tableRatedA ? `${tableRatedA} A` : "—", isDirect200FiveFive ? "60 A" : (rowByBasis.fuji?.breakerRatedA != null ? `${rowByBasis.fuji.breakerRatedA} A` : "—")],
                  ["MCCB", naisenBreakerA != null ? `${naisenBreakerA} A` : "—", rowByBasis.company?.breakerModel ?? "型番要確認", mitsuMccb && tableRatedA ? `${mitsuMccb.model}\nIcu ${mitsuMccb.icu} kA` : "—", isDirect200FiveFive ? "BW63SAG-3P 060\nIcu 10 kA" : (fujiMccb && tableRatedA ? `${fujiMccb.model}\nIcu ${fujiMccb.icu} kA` : "—")],
                  ["ELCB", naisenBreakerA != null ? `${naisenBreakerA} A` : "—", "型番要確認", mitsuElcb && tableRatedA ? `${mitsuElcb.model}\nIcu ${mitsuElcb.icu} kA` : "—", isDirect200FiveFive ? "EW63SAG-3P 060\nIcu 10 kA" : (fujiElcb && tableRatedA ? `${fujiElcb.model}\nIcu ${fujiElcb.icu} kA` : "—")],
                  ["電磁接触器", "JIS C 8201-4-1", "—", isDirect200FiveFive ? "S-T25" : (rowByBasis.mitsubishi?.contactorModel ?? "要確認"), isDirect200FiveFive ? "SC-NEXT SW26XA" : (rowByBasis.fuji?.contactorModel ?? "要確認")],
                  ["電磁開閉器", "JIS C 8201-4-1", rowByBasis.company?.contactorModel ?? "—", isDirect200FiveFive ? "MSO-T25" : (rowByBasis.mitsubishi?.contactorModel ?? "要確認"), isDirect200FiveFive ? "SW26XA-□◇T018" : (rowByBasis.fuji?.contactorModel ?? "要確認")],
                  [copy.thermal, "JIS C 8201-4-1", rowByBasis.company?.thermalModel ?? "—", isDirect200FiveFive ? "TH-T25\n22 A（18～26 A）" : (rowByBasis.mitsubishi?.thermalModel ?? (rowByBasis.mitsubishi?.thermalSettingA != null ? `${rowByBasis.mitsubishi.thermalSettingA} A` : "要確認")), isDirect200FiveFive ? "18～24 A" : (rowByBasis.fuji?.thermalModel ?? (rowByBasis.fuji?.thermalSettingA != null ? `${rowByBasis.fuji.thermalSettingA} A` : "要確認"))],
