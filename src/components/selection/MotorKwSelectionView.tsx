        ...
        {selectedKw == null ? (
          ...
          <div className="data-table-wrap mt-3">
            <table className="data-table table-fixed" style={{ minWidth: 900 }}>
              ...
              <tbody>
                {[
                  [copy.rated, naisen ? `${naisen.conventionalA} A（公開資料の参考値・要原本照合）` : "対象値なし", rowByBasis.company?.ratedCurrentA != null ? `${rowByBasis.company.ratedCurrentA} A` : "—", mitsu ? `${mitsu.ratedCurrentA} A` : rowByBasis.mitsubishi?.ratedCurrentA != null ? `${rowByBasis.mitsubishi.ratedCurrentA} A` : "—", fuji ? `${fuji.ratedCurrentA} A` : rowByBasis.fuji?.ratedCurrentA != null ? `${rowByBasis.fuji.ratedCurrentA} A` : "—"],
                  [copy.starting, "JEAC原本確認待ち", rowByBasis.company?.startingCurrentA != null ? `${rowByBasis.company.startingCurrentA} A` : "—", mitsu ? `${mitsu.startingConditionA} A（選定条件：全負荷電流×${mitsu.startingMultiplier}）` : "メーカー表の選定条件を確認", fuji ? `${fuji.startingCurrentA} A` : rowByBasis.fuji?.startingCurrentA != null ? `${rowByBasis.fuji.startingCurrentA} A` : "—"],
                  ...
                  [method === "starDelta" ? "電磁接触器 MC-M / MC-S / MC-Δ" : "電磁接触器", "JIS C 8201-4-1", "—", method === "inverter" ? "—（入力側MCは運用条件による）" : mitsu ? `${mitsu.ratedCurrentA} A（適用電動機全負荷電流）\n適用フレーム: ${mitsu.contactorFrame ?? "Y-Δ別構成"}` : "要確認", method === "inverter" ? "—（入力側MCは運用条件による）" : method === "starDelta" && fuji ? `MC-M: ${fuji.mainContactor}\nMC-S: ${fuji.starContactor}\nMC-Δ: ${fuji.deltaContactor}` : fuji ? `${fuji.ratedCurrentA} A（適用電動機全負荷電流）\n単体形式はSC-NEXT選定表の対象外` : "SC-NEXT適用外"],
                  ["電磁開閉器", "JIS C 8201-4-1", rowByBasis.company?.contactorModel ?? "—", method === "inverter" ? "—" : mitsu ? `${mitsu.heaterA} A\n適用フレーム: ${mitsu.contactorFrame ?? "Y-Δ別構成"}` : "要確認", method === "starDelta" || method === "inverter" ? "—" : fuji ? `${fuji.ratedCurrentA} A（${fuji.heatRange}）\n${fuji.switchModel}` : "SC-NEXT適用外"],
                  [copy.thermal, "JIS C 8201-4-1", rowByBasis.company?.thermalModel ?? "—", method === "inverter" ? "—（電子サーマル設定）" : mitsu ? `${mitsu.heaterA} A（ヒータ称呼）\n適用フレーム: ${mitsu.contactorFrame ?? "Y-Δ別構成"}` : "要確認", method === "inverter" ? "—（電子サーマル設定）" : fuji ? `${fuji.ratedCurrentA} A（${fuji.heatRange}）\n${method === "starDelta" ? fuji.olrModel : fuji.switchModel?.match(/T[^-]+$/)?.[0] ?? "ヒートエレメント"}` : "SC-NEXT適用外"],
                  ...
                  [copy.wire, "JEAC8001-2022", rowByBasis.company?.wireSize ?? "—", rowByBasis.mitsubishi?.wireSize ?? "要確認", fuji ? (method === "starDelta" ? `電源側 ${fuji.wire}\n負荷側 ${fuji.loadWire}` : fuji.wire) : rowByBasis.fuji?.wireSize ?? "要確認"],
