            ...
            <table className="data-table table-fixed" style={{ minWidth: 900 }}>
              ...
              <tbody>
                {[
                  ...
                  [method === "starDelta" ? "電磁接触器 MC-M / MC-S / MC-Δ" : "電磁接触器", "JIS C 8201-4-1", "—", method === "inverter" ? "—（入力側MCは運用条件による）" : method === "starDelta" && mitsu ? `MC-M: ${mitsu.mainContactor}\nMC-S: ${mitsu.starContactor}\nMC-Δ: ${mitsu.deltaContactor}` : mitsu ? `${mitsu.ratedCurrentA} A\n${mitsu.contactorModel}` : "要確認", method === "inverter" ? "—（入力側MCは運用条件による）" : method === "starDelta" && fuji ? `MC-M: ${fuji.mainContactor}\nMC-S: ${fuji.starContactor}\nMC-Δ: ${fuji.deltaContactor}` : fuji ? `${fuji.ratedCurrentA} A\n${fuji.switchModel}` : "メーカー選定表に該当なし"],
                  ["電磁開閉器", "JIS C 8201-4-1", rowByBasis.company?.contactorModel ?? "—", method === "starDelta" || method === "inverter" ? "—" : mitsu ? `${mitsu.heaterA} A（${mitsu.heaterRange} A）\n${mitsu.starterModel}` : "要確認", method === "starDelta" || method === "inverter" ? "—" : fuji ? `${fuji.ratedCurrentA} A（${fuji.heatRange}）\n${fuji.switchModel}` : "メーカー選定表に該当なし"],
                  [copy.thermal, "JIS C 8201-4-1", rowByBasis.company?.thermalModel ?? "—", method === "inverter" ? "—（電子サーマル設定）" : mitsu ? `${mitsu.heaterA} A（${mitsu.heaterRange} A）\n${mitsu.thermalModel}` : "要確認", method === "inverter" ? "—（電子サーマル設定）" : fuji ? `${fuji.ratedCurrentA} A（${fuji.heatRange}）\n${fujiThermalLabel}` : "メーカー選定表に該当なし"],
                  ...
                  [copy.source, "JEAC8001-2022 表3705-1 / 資料3-7-3（原本照合要）", rowByBasis.company?.remarks ?? copy.companyMissing, mitsu ? "三菱 MS-T/N 総合カタログ＋WS-V 表4-11～4-14" : rowByBasis.mitsubishi?.source?.title ?? copy.noData, fuji ? `富士 ${fuji.catalog} 公式選定表` : rowByBasis.fuji?.source?.title ?? copy.noData],
