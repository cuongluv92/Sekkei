...
import { getWeightShape, type WeightDimKey, type WeightDims, type WeightShapeKey } from "@/lib/utils/weightShapes";
 ...
 * triplicated.
 */
export function WeightShapeCalcSection({ shapeKey, materials }: WeightShapeCalcSectionProps) {
  const { t } = useTranslation();
  const shape = getWeightShape(shapeKey);

  const [dimRaw, setDimRaw] = useState<Record<WeightDimKey, string>>({ W: "", H: "", t1: "", t2: "" });
  ...
  const dimStates = useMemo(
    () => Object.fromEntries(shape.fields.map((k) => [k, parseField(dimRaw[k])])) as Partial<Record<WeightDimKey, number | null | "">>,
    [shape.fields, dimRaw],
  );
  ...
  const unitWeight = area !== null && lengthValid && densityValid ? (area * (lengthState as number) * (densityState as number)) / 1e6 : null;
  const totalWeight = unitWeight !== null && quantityValid ? unitWeight * (quantityState as number) : null;
      ...
      <div className="panel-body grid grid-cols-1 gap-5 lg:grid-cols-2">
        ...
        <div className="flex flex-col gap-3.5">
          <div>
            ...
            <select value={materialId} onChange={(e) => handleMaterialChange(e.target.value)} className="field-input">
          ...
          <div>
            ...
            <input
              ...
              placeholder="7.85"
              className={densityState === null ? "field-input !border-danger" : "field-input"}
            />
          </div>

          <div>
            <span className="field-label">{t("weightCalc.basic.dimensions")}</span>
              ...
              {ALL_DIM_KEYS.filter((k) => shape.fields.includes(k)).map((k) => (
                <div key={k}>
                  ...
                  <input
                    ...
                    step="0.1"
                    value={dimRaw[k]}
                    onChange={(e) => setDimRaw((prev) => ({ ...prev, [k]: e.target.value }))}
          ...
          <div className="grid grid-cols-2 gap-2.5">
            <div>
              <label className="field-label">{t("weightCalc.basic.length")}</label>
              <input
                type="number"
                step="1"
                value={lengthRaw}
                onChange={(e) => setLengthRaw(e.target.value)}
                className={lengthState === null ? "field-input !border-danger" : "field-input"}
              />
            </div>
            <div>
              <label className="field-label">{t("weightCalc.basic.quantity")}</label>
              <input
                type="number"
                step="1"
                min="1"
                ...
                className={quantityState === null ? "field-input !border-danger" : "field-input"}
              />
            </div>
          </div>
          ...
          quantityState === null ||
          densityState === null ? (
            <p className="text-[11.5px] text-danger">{t("weightCalc.basic.invalidInput")}</p>
          ) : null}

          <div className="grid grid-cols-3 gap-2.5 border-t border-border pt-3.5">
            <ResultTile label={t("weightCalc.basic.sectionArea")} value={area !== null ? `${roundTo(area, 2)}` : "—"} unit="mm²" />
            <ResultTile
          ...
          {missingMaterial && area !== null && (
            <p className="text-[11.5px] text-muted-2">{t("weightCalc.basic.noMaterial")}</p>
          )}
            ...
            <div className="flex flex-col gap-1 font-mono text-[12px] text-muted">
              ...
              {dimsValid && (
                <span className="text-foreground">
                  {shape.areaFormulaSubstituted(dims)} = {roundTo(area as number, 2)} mm²
                </span>
              )}
              <span className="mt-1.5">{t("weightCalc.basic.unitWeightFormula")}</span>
              {unitWeight !== null && (
                <span className="text-foreground">
                  {t("weightCalc.basic.unitWeight")} = {roundTo(area as number, 2)} × {lengthState} × {densityState} × 10⁻⁶ ={" "}
                  {roundTo(unitWeight, 3)} kg
                </span>
              )}
              <span className="mt-1.5">{t("weightCalc.basic.totalWeightFormula")}</span>
              {totalWeight !== null && (
                <span className="text-foreground">
                  {t("weightCalc.basic.totalWeight")} = {roundTo(unitWeight as number, 3)} × {quantityState} ={" "}
...
function ResultTile({
  ...
  return (
    <div className="rounded-lg border border-border bg-surface-2 px-3 py-2.5">
      <div className="text-[10.5px] tracking-wide text-muted uppercase">{label}</div>
      <div className={emphasize ? "mt-0.5 text-[17px] font-bold text-accent" : "mt-0.5 text-[15px] font-bold text-foreground"}>
