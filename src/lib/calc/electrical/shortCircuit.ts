...
/**
 * 計算された短絡電流Iscと、ユーザー自身が入力した遮断器の定格遮断電流とを
 * 比較するだけの単純な算術判定。定格遮断電流は必ず対象機器の同一定格電圧・
 * 同一条件で読み取った値を使用すること（異なる電圧・機種の値と混同しない）。
 * 上記の通りIsc自体が変圧器%Zのみの簡易値（上位系統・ケーブル等未考慮）で
 * あるため、この判定だけで遮断器選定の最終「OK/NG」を決めてはならない —
 * 必ず詳細検討・メーカー資料で確認すること。定格遮断電流の基準値そのものを
 * 本システムが規定・推測することはない。
 */
