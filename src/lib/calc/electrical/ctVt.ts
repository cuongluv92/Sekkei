...
/**
 * CTの精度階級は、当システムが規格原文の該当表を直接確認できていないため
 * （計測用0.1級〜3.0級・保護用5P/10P等の呼称は複数の実務資料・教科書で
 * 広く見かけるが、正式な数値・区分は未検証）、選択式の候補リストとしては
 * 一切提示しない — 選択肢を並べること自体が「この一覧が正しい／網羅的」
 * という誤解を招くため。UIでは自由記述の参考メモ欄のみを提供し、対象機器の
 * 銘板・試験成績書に記載された呼称をそのままユーザー自身に入力してもらう。
 */
export const CT_ACCURACY_CLASS_NOTE: Record<InstrumentTransformerPurpose, string> = {
  measurement:
    "計測用CTの精度階級（例: 0.5級、1.0級等）は、対象機器の銘板・試験成績書に記載された呼称をご自身で入力してください。" +
    "当システムは精度階級の正式な区分・許容誤差の数値を規格原文（JIS C 1732-2）で直接確認していないため、選択肢の一覧は提示しません。",
  protection:
    "保護用CTの過電流特性（例: 5P10等）は、対象機器の銘板・試験成績書に記載された呼称をご自身で入力してください。" +
    "該当規格番号自体が当システムで未特定（要確認）のため、選択肢の一覧は提示しません。計測用の精度階級を保護用に流用しないでください。",
};
